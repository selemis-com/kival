...
use crate::{
    commands::objects::{ObjectCommandError, object_error_codes},
    utils::{
...
object_error_codes! {
    pub(crate) enum CommentListErrorCode {
        failures {
...
type CommentListError = ObjectCommandError<CommentListErrorCode>;

object_error_codes! {
    pub(crate) enum CommentReadErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type CommentReadError = ObjectCommandError<CommentReadErrorCode>;

object_error_codes! {
    pub(crate) enum CommentWriteErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type CommentWriteError = ObjectCommandError<CommentWriteErrorCode>;

object_error_codes! {
    pub(crate) enum CommentMutationErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type CommentMutationError = ObjectCommandError<CommentMutationErrorCode>;
...
pub enum CommentsSubcommand {
    ...
    ///
    /// With `--input`, the JSON object requires `body` (string) and accepts `mentioned_user_ids`
    /// (array of user UUIDs; defaults to empty). Unknown properties are rejected.
    #[argx(
        metadata({
            "readOnly": false,
            "destructive": false,
    ...
    /// parent. With `--input`, the JSON object requires `body` (string) and accepts
    /// `mentioned_user_ids` (array of user UUIDs; defaults to empty). Unknown properties are
    /// rejected.
    #[argx(
        metadata({
            "readOnly": false,
    ...
    ///
    /// With `--input`, the JSON object requires `body` and accepts `mentioned_user_ids` as an array
    /// of user UUIDs. Unknown properties are rejected.
    #[argx(
        metadata({
            "readOnly": false,
            "destructive": false,
...
impl CommentsListCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<CommentsListOutput, CommentListError> {
...
impl CommentsMentionsCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ListResponse<CommentMentionCandidate>, CommentReadError> {
        let client = authenticated_client(&ctx)?;
        let response = client
...
impl CommentsCreateCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<CommentThread, CommentWriteError> {
...
impl CommentsReplyCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<Comment, CommentWriteError> {
        let target = self.target;
...
impl CommentsDeleteCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<Comment, CommentMutationError> {
        let client = authenticated_client(&ctx)?;
        let comment = client
...
impl CommentsResolveCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<CommentThread, CommentMutationError> {
        let client = authenticated_client(&ctx)?;
        let thread = client
...
fn comment_write_input(
    ...
    let body = body.ok_or_else(|| CliFailure::invalid_argument("comment body is required"))?;
