...
pub(super) fn parse_object_document(input: &str) -> Result<ObjectDocument> {
    ...
    let title = title.ok_or_else(|| {
        ObjectError::invalid_argument("missing required object front-matter field `title`")
    })?;
    let metadata = metadata.ok_or_else(|| {
        ObjectError::invalid_argument("missing required object front-matter field `metadata`")
    })?;
...
fn split_front_matter(input: &str) -> Result<(&str, &str)> {
    ...
    let end = remainder.find(&delimiter).ok_or_else(|| {
        ObjectError::invalid_argument("edited object front matter must end with a `---` line")
    })?;
