...
object_error_codes! {
    pub(crate) enum ObjectGraphErrorCode {
        failures {
...
type ObjectGraphError = ObjectCommandError<ObjectGraphErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectPagedRelationErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectPagedRelationError = ObjectCommandError<ObjectPagedRelationErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectEdgeReadErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectEdgeReadError = ObjectCommandError<ObjectEdgeReadErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectEdgeMutationErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectEdgeMutationError = ObjectCommandError<ObjectEdgeMutationErrorCode>;
...
impl ObjectEdgesListCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ListResponse<ObjectEdge>, ObjectPagedRelationError> {
        let client = authenticated_client(&ctx)?;
        let response = client
            .list_object_edges(
                self.target.workspace_id,
...
impl ObjectEdgesCreateCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectEdge, ObjectEdgeMutationError> {
        let client = authenticated_client(&ctx)?;
        let edge = client
            .create_object_edge(
                self.workspace_id,
...
impl ObjectEdgesGetCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectEdge, ObjectEdgeReadError> {
        let client = authenticated_client(&ctx)?;
