...
use super::{
    ObjectCommandError, ObjectError, ObjectErrorCode, ObjectTargetArgs,
...
object_error_codes! {
    pub(crate) enum ObjectListErrorCode {
        failures {
...
type ObjectListError = ObjectCommandError<ObjectListErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectReadErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectReadError = ObjectCommandError<ObjectReadErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectEditErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
            ...
            VersionConflict => ("version.conflict", VersionConflict),
            ServerUnavailable => ("server.unavailable", ServerUnavailable),
            RateLimited => ("rate_limited", RateLimited),
            RequestFailed => ("request.failed", RequestFailed),
            InvalidField => ("output.invalid_field", InvalidField),
        ...
        objects {
            ObjectNotFound => ("object.not_found", NotFound),
            ObjectArchived => ("object.archived", Archived),
        }
    }
}

/// Error returned by the corresponding command handler.
type ObjectEditError = ObjectCommandError<ObjectEditErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectUpdateErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectUpdateError = ObjectCommandError<ObjectUpdateErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectLifecycleMutationErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectLifecycleMutationError = ObjectCommandError<ObjectLifecycleMutationErrorCode>;

object_error_codes! {
    pub(crate) enum ObjectCreateErrorCode {
        failures {
            AuthenticationRequired => ("authentication.required", AuthenticationRequired),
            PermissionDenied => ("permission.denied", PermissionDenied),
            InvalidArgument => ("invalid.argument", InvalidArgument),
...
/// Error returned by the corresponding command handler.
type ObjectCreateError = ObjectCommandError<ObjectCreateErrorCode>;
...
#[derive(Debug, Args)]
#[argx(one_of = ["input", "workspace_id"])]
pub struct ObjectsCreateCommand {
    ...
    #[argx(long, conflicts = ["body_file", "input"])]
    pub body: Option<String>,
...
#[derive(Debug, Args)]
#[argx(
    one_of = ["input", "expected_current_version_id"],
    any_of = [
        "input",
        "title",
        "body",
...
impl ObjectsBodyCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectBodyOutput, ObjectReadError> {
        ...
        let version = response
            .current_version
            .ok_or_else(|| ObjectError::invalid_argument("object has no current version"))?;
...
impl ObjectsEditCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectEditOutput, ObjectEditError> {
        let client = authenticated_client(&ctx)?;
            ...
            .await
        {
            ...
            Err(error) => {
                let body = ObjectError::from_client_error(&error);
...
fn editable_version(response: &ObjectResponse) -> Result<&ObjectVersion> {
    if response.object.status != ArchiveStatus::Active {
        return Err(ObjectError::object(
            ...
            "Archived objects cannot be edited.",
            None,
        )
        .into());
    }
    ...
    if !matches!(response.effective_role, ObjectRole::Editor | ObjectRole::Admin) {
        return Err(ObjectError::common(
            ...
            "Object requires editor or admin access to edit.",
            None,
        )
        .into());
    }
    ...
    response.current_version.as_ref().ok_or_else(|| {
        ObjectError::invalid_argument("object has no current version to edit").into()
    })
}
...
fn object_document(version: &ObjectVersion) -> Result<ObjectDocument> {
    let metadata = version.metadata.as_object().cloned().ok_or_else(|| {
        ObjectError::invalid_argument("current object metadata is not a JSON object")
    })?;
...
fn local_edit_error(message: &str, path: &Path) -> ObjectError {
    ObjectError::common(
...
fn edit_recovery_error(body: &ObjectError, path: &Path) -> ObjectError {
    ObjectError {
        code: body.code,
...
fn object_edit_output(response: &ObjectResponse, changed: bool) -> Result<ObjectEditOutput> {
    let version_id =
        response.current_version.as_ref().map(|version| version.id).ok_or_else(|| {
            ObjectError::invalid_argument("object has no current version after edit")
        })?;
...
impl ObjectsCreateCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectResponse, ObjectCreateError> {
        let input = self.into_input()?;
        let title = input.title.trim();
        if title.is_empty() {
            return Err(ObjectError::invalid_argument("title must not be empty").into());
        }
        let client = authenticated_client(&ctx)?;
        let response = client
            .create_object(
    ...
    fn into_input(self) -> Result<CreateObjectInput> {
        ...
        let workspace_id = self
            .workspace_id
            .ok_or_else(|| ObjectError::invalid_argument("workspace ID is required"))?;
        let title = self.title.ok_or_else(|| ObjectError::invalid_argument("title is required"))?;
...
impl ObjectsUpdateCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectResponse, ObjectUpdateError> {
        ...
        {
            return Err(ObjectError::invalid_argument("at least one field must be provided").into());
        }
    ...
    fn into_input(self) -> Result<UpdateObjectInput> {
        ...
        let expected_current_version_id = self.expected_current_version_id.ok_or_else(|| {
            ObjectError::invalid_argument("expected current version ID is required")
        })?;
    ...
    values
        ...
        .map(|assignment| {
            let (key, raw_value) = assignment
                .split_once('=')
                .ok_or_else(|| ObjectError::invalid_argument("--metadata-set must use KEY=JSON"))?;
...
impl ObjectsArchiveCommand {
    ...
    pub(crate) async fn run(
        ...
        ctx: CliContext,
        output: OutputMode,
    ) -> std::result::Result<ObjectResponse, ObjectLifecycleMutationError> {
        let client = authenticated_client(&ctx)?;
        let response =
...
mod tests {
    ...
    fn editable_version_enforces_status_and_role() {
        ...
        let viewer_error =
            editable_version(&active_viewer).unwrap_err().downcast::<ObjectError>().unwrap();
        ...
        let archived_error =
            editable_version(&archived_admin).unwrap_err().downcast::<ObjectError>().unwrap();
