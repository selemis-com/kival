...
fn parse_editor_command(value: &str) -> Result<EditorCommand> {
    ...
    if quote.is_some() {
        return Err(
            CliFailure::invalid_argument("editor command contains an unmatched quote").into()
        );
    }
    if token_started {
    ...
    let Some(program) = parts.next() else {
        return Err(CliFailure::invalid_argument("editor command must not be empty").into());
    };
...
mod tests {
    ...
    fn edit_body_retains_file_when_editor_fails() {
        ...
        let error = edit_document_at(path.clone(), "original", |_path| {
            Err(CliFailure::invalid_argument("editor failed intentionally").into())
        })
        .unwrap_err();
