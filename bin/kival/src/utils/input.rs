...
fn read_input<T>(
    ...
    match source {
        InputPath::File(path) => {
            let file =
                File::open(&path).map_err(|_error| CliError::input_read_failed(Some(&path)))?;
...
fn decode_text(reader: &mut dyn std::io::Read, path: Option<&Path>) -> Result<String> {
    ...
    reader.read_to_string(&mut value).map_err(|_error| CliError::input_read_failed(path))?;
    Ok(value)
}
...
pub fn reject_conflicting_input(input: &Option<InputPath>, fields: &[(&str, bool)]) -> Result<()> {
    ...
    if conflicts.is_empty() {
        ...
        Err(CliError::input_conflicting_sources(&conflicts).into())
    }
}
...
fn map_json_error(error: &serde_json::Error, path: Option<&Path>, field_path: String) -> CliError {
    let details = error_details(error, field_path);
    match error.classify() {
        Category::Io => CliError::input_read_failed(path),
        Category::Data => CliError::input_invalid_value(details),
        Category::Syntax | Category::Eof => CliError::input_invalid_json(details),
    }
}
...
mod tests {
    ...
    fn read_text_input_reports_missing_file() {
        ...
        assert_eq!(error.code, CliErrorCode::InputReadFailed);
    }
    ...
    fn decode_text_reports_reader_io_errors_as_read_failed() {
        ...
        let error = decode_text(&mut reader, None).unwrap_err().downcast::<CliError>().unwrap();
    ...
    fn decode_json_rejects_trailing_json_values() {
        ...
        assert_eq!(error.code, CliErrorCode::InputInvalidJson);
    }
    ...
    fn read_json_input_reports_invalid_value() {
        ...
        assert_eq!(error.code, CliErrorCode::InputInvalidValue);
        let _ = fs::remove_file(path);
    }
    ...
    fn reject_conflicting_input_reports_payload_fields() {
        ...
        assert_eq!(error.code, CliErrorCode::InputConflictingSources);
