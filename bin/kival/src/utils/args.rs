...
pub fn grant_principal(user_id: Option<Uuid>, group_id: Option<Uuid>) -> Result<GrantPrincipal> {
    match (user_id, group_id) {
        ...
        (None, None) => Err(CliFailure::invalid_argument("one principal must be provided").into()),
        (Some(_), Some(_)) => {
            Err(CliFailure::invalid_argument("only one principal may be provided").into())
        }
    }
}
...
pub fn metadata_value(metadata: Option<&str>) -> Result<Value> {
    let value = match metadata {
        Some(value) => serde_json::from_str(value).map_err(|error| {
            CliFailure::invalid_argument(format!("metadata must be valid JSON: {error}"))
        })?,
        None => Value::Object(Map::new()),
    };

    let Value::Object(metadata) = &value else {
        return Err(CliFailure::invalid_argument("metadata must be a JSON object").into());
    };
...
pub fn validate_flat_metadata_member(key: &str, value: &Value) -> Result<()> {
    ...
    if valid {
        ...
        Err(CliFailure::invalid_argument(format!(
