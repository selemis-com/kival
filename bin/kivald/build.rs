...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("cargo:rustc-env=KIVAL_CARGO_FEATURES={features}");

    let target_triple = env::var("TARGET")?;
    println!("cargo:rustc-env=KIVAL_CARGO_TARGET_TRIPLE={target_triple}");
    println!("cargo:rustc-env=KIVAL_GIT_SHA={sha}");
    println!("cargo:rustc-env=KIVAL_GIT_SHA_SHORT={}", &sha[..8]);
