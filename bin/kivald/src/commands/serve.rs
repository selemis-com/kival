...
impl ServeCommand {
    ...
    pub async fn run(&self, ctx: CliContext, quiet: bool) -> Result<Duration> {
        ...
        let default_config_path = ctx.datadir.join(DEFAULT_CONFIG_FILENAME);
        let config_path = if let Some(path) = config.as_deref() {
            Some(path)
        } else if default_config_path.try_exists()? {
        ...
        let config = match ServerConfig::load(config_path) {
            ...
            Err(err) => {
                if let Some(path) = config_path {
                    error!(target: "kival::cli", path = %path.display(), error = ?err, "Failed to load configuration");
                } else {
                    error!(target: "kival::cli", error = ?err, "Failed to load configuration");
                }
                return Err(err);
            }
        };
        ...
        if let Some(path) = config_path {
            info!(target: "kival::cli", "Configuration loaded from: {}", path.display());
        }
        ...
        let db_pool = open_pool_with_settings(connect_options()?, database_pool_settings).await?;
