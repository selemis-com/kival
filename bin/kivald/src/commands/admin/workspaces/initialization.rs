...
pub(super) struct InitializedWorkspace {
    ...
    /// Bootstrapped administrator assigned to the workspace at creation.
    pub(super) bootstrap_admin_user_id: Uuid,
    /// Username of the bootstrapped administrator.
    pub(super) bootstrap_admin_username: String,
}
...
struct DemoRecipe {
    ...
    /// Objects pinned for the bootstrap administrator after the scenario is seeded.
    #[serde(default)]
    bootstrap_admin_pins: Vec<String>,
    /// Objects favorited for the bootstrap administrator after the scenario is seeded.
    #[serde(default)]
    bootstrap_admin_favorites: Vec<String>,
...
struct DemoAccessContext<'a> {
    ...
    workspace_id: Uuid,
    /// Bootstrap administrator attributed as the grant administrator.
...
pub(super) async fn create_workspace_as_operator(
    ...
    let Some((bootstrap_admin_user_id, bootstrap_admin_username, bootstrap_admin_disabled)) =
        lock_bootstrap_admin(&mut tx)
            .await
            .wrap_err("failed to resolve bootstrap administrator")?
    else {
        bail!("Kival is not bootstrapped; run `kivald admin bootstrap` first");
    };
    if bootstrap_admin_disabled {
        bail!("bootstrap administrator is disabled");
    }

    let created =
        create_workspace(&mut tx, name, description.map(str::trim), bootstrap_admin_user_id)
            .await
    ...
    emit(
        ...
        EventInsert::new(
            ...
            json!({
                "workspace_membership_id": created.creator_membership_id,
                "workspace_role": MembershipRole::Admin.as_str(),
            }),
        )
        .workspace(workspace.id)
        .target_user(bootstrap_admin_user_id),
    )
    .await?;
    ...
    match resolved {
        ...
        Some(ResolvedInitializer::Template(recipe)) => {
            execute_template(&mut tx, workspace.id, bootstrap_admin_user_id, recipe).await?;
        }
        Some(ResolvedInitializer::Demo(recipe)) => {
            execute_demo(&mut tx, workspace.id, bootstrap_admin_user_id, recipe).await?;
        }
    }
...
/// Executes a reusable template as the bootstrapped workspace administrator.
async fn execute_template(
    ...
    let context = SeedExecutionContext { workspace_id, actor_id: bootstrap_admin_user_id };
...
async fn execute_demo(
    ...
    let actors =
        provision_demo_actors(tx, workspace_id, bootstrap_admin_user_id, &recipe.actors).await?;
    let groups =
        provision_demo_groups(tx, workspace_id, bootstrap_admin_user_id, &actors, &recipe.groups)
            .await?;
    ...
    seed_bootstrap_admin_object_markers(
        tx,
        workspace_id,
        bootstrap_admin_user_id,
        &objects,
        &recipe.bootstrap_admin_pins,
        &recipe.bootstrap_admin_favorites,
    )
    .await?;
...
async fn provision_demo_groups(
    ...
    for (key, configured) in groups {
        ...
        let workspace_group =
            create_workspace_group(tx, workspace_id, group.id, bootstrap_admin_user_id)
                .await
        ...
        for (actor_key, role) in &configured.members {
            ...
            .await
            .wrap_err_with(|| format!("failed to add demo actor {actor_key:?} to group {key:?}"))?;
            emit(
                tx,
                EventInsert::new(
                    bootstrap_admin_user_id,
...
/// Seeds personal pins and favorites for the bootstrap administrator.
async fn seed_bootstrap_admin_object_markers(
    tx: &mut Transaction<'_, Postgres>,
    workspace_id: Uuid,
    bootstrap_admin_user_id: Uuid,
    ...
    for key in pins {
        let object = objects.get(key).ok_or_else(|| {
            eyre!("demo bootstrap administrator pin references unknown object {key:?}")
        })?;
        sqlx::query(
            r#"
    ...
    for key in favorites {
        let object = objects.get(key).ok_or_else(|| {
            eyre!("demo bootstrap administrator favorite references unknown object {key:?}")
        })?;
        sqlx::query(
            r#"
...
async fn seed_demo_object_access(
    ...
    for configured in &profile.grants {
        ...
        let grant = create_object_grant(
            ...
            principal,
            role,
            context.bootstrap_admin_user_id,
        )
        .await
...
fn validate_catalog(catalog: &Catalog) -> Result<()> {
    ...
    for recipe in &catalog.demos {
        ...
        validate_demo_bootstrap_admin_markers(recipe, &actions)?;
    }

    Ok(())
}

/// Validates demo bootstrap administrator pins and favorites against objects declared by the
/// recipe.
fn validate_demo_bootstrap_admin_markers(
    recipe: &DemoRecipe,
    actions: &[SeedAction],
) -> Result<()> {
    ...
    for (kind, keys) in
        [("pin", &recipe.bootstrap_admin_pins), ("favorite", &recipe.bootstrap_admin_favorites)]
    {
        let mut seen = HashSet::new();
        for key in keys {
            if !object_keys.contains(key.as_str()) {
                bail!(
                    "demo {:?} bootstrap administrator {kind} references unknown object {key:?}",
                    recipe.id
                );
            }
            if archived_object_keys.contains(key.as_str()) {
                bail!(
                    "demo {:?} bootstrap administrator {kind} references archived object {key:?}",
                    recipe.id
                );
            }
            if !seen.insert(key.as_str()) {
                bail!(
                    "demo {:?} declares duplicate bootstrap administrator {kind} for object {key:?}",
                    recipe.id
                );
            }
        }
...
mod tests {
    ...
    #[sqlx::test(migrations = "../../crates/kernel/migrations")]
    async fn acme_demo_seeds_bootstrap_admin_markers_commentary_and_sparse_graph(
        pool: sqlx::PgPool,
    ) -> eyre::Result<()> {
        use kival_kernel::{
            create_user, grant_global_admin_as_operator, record_bootstrap_completed,
