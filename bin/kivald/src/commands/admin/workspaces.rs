...
impl AdminWorkspaceCreateCommand {
    ...
    async fn run(&self, db_pool: PgPool) -> Result<()> {
        ...
        info!(
            ...
            bootstrap_admin_user_id = %created.bootstrap_admin_user_id,
            bootstrap_admin_username = %created.bootstrap_admin_username,
            initializer = ?initializer,
            "Created Kival workspace",
        );
        println!(
            "Created workspace {} for {}.",
            created.workspace_id, created.bootstrap_admin_username
        );
