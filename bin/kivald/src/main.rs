...
use argx::{Defaults, Dotenv, Environment, Parser, Subcommand, Toml};
use kival_cli::{
...
/// Default dotenv file read from the current working directory when present.
...
impl ServerConfig {
    /// Loads effective server configuration from defaults, optional dotenv and TOML files, and the
    /// process environment.
    ///
    /// # Errors
    ///
    ...
    pub fn load(path: Option<&Path>) -> eyre::Result<Self> {
        let mut loader = Self::loader().layer(Defaults);
