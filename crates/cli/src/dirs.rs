...
///
/// This is useful for when a path is optional, such as the `--datadir` flag.
...
impl<D: XdgPath> Display for MaybePlatformPath<D> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match &self.0 {
            ...
            // Typed argument defaults are rendered and parsed through this representation,
            // so `default` must round-trip through `Display -> FromStr`.
