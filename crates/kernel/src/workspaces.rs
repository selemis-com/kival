...
/// Result of creating a workspace and its initial administrator membership.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CreatedWorkspace {
    /// Created workspace.
    pub workspace: WorkspaceRow,
    /// Initial administrator membership identifier.
    pub creator_membership_id: Uuid,
}
...
async fn create_workspace_in_savepoint(
    ...
    .await?
    .try_into()?;
    let creator_membership_id = sqlx::query_scalar::<_, Uuid>(
        r#"
    ...
    Ok(CreatedWorkspace { workspace, creator_membership_id })
}
