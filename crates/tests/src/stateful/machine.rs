...
pub enum Operation {
    ...
    CreateWorkspace {
        ...
        output: Handle,
        /// Handle to bind to the creator's initial administrator membership.
        membership_output: Handle,
        /// Workspace name.
        name: String,
    },
...
impl ReferenceStateMachine for KivalStateMachine {
    ...
    fn apply(mut state: Self::State, transition: &Self::Transition) -> Self::State {
        match transition {
            Operation::CreateWorkspace { actor, output, membership_output, name } => {
                state.next_workspace = state.next_workspace.max(output.index.saturating_add(1));
                state.next_membership =
                    state.next_membership.max(membership_output.index.saturating_add(1));
                state.workspaces.insert(*output, Lifecycle::Active);
                state.workspace_names.insert(*output, name.clone());
                state
                    .memberships
                    .insert(*membership_output, (*output, *actor, MembershipRole::Admin, true));
                state.record_event(
            ...
            Operation::ArchiveWorkspace { actor, workspace } => {
                if let Some(lifecycle) = state.workspaces.get_mut(workspace) {
    ...
    fn preconditions(state: &Self::State, transition: &Self::Transition) -> bool {
        match transition {
            ...
            Operation::CreateWorkspace { output, membership_output, .. } => {
                output.kind == ResourceKind::Workspace
                    && state.workspace(*output).is_none()
                    && membership_output.kind == ResourceKind::Membership
                    && !state.memberships.contains_key(membership_output)
                    && !state.group_memberships.contains_key(membership_output)
            }
...
fn selected_workspace_read(state: &Model) -> Option<BoxedStrategy<Operation>> {
    let workspaces: Vec<_> = state.workspaces();
    if workspaces.is_empty() {
        None
    } else {
        Some(
...
fn selected_unauthorized_workspace_mutation_probe(
    ...
    let mut candidates = Vec::new();
    for workspace in state.workspaces_with(Lifecycle::Active) {
        for actor in Actor::ALL {
