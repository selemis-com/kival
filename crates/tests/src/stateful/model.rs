...
pub struct Model {
    ...
    /// Lifecycle of every modeled workspace.
    pub(super) workspaces: BTreeMap<Handle, Lifecycle>,
...
impl Model {
    ...
    pub fn workspace(&self, handle: Handle) -> Option<Lifecycle> {
        self.workspaces.get(&handle).copied()
    }
    ...
    /// Returns workspace handles in `lifecycle`.
    pub(super) fn workspaces_with(&self, lifecycle: Lifecycle) -> Vec<Handle> {
        self.workspaces
            .iter()
            .filter_map(|(handle, current)| (*current == lifecycle).then_some(*handle))
            .collect()
    }

    /// Returns all known workspace handles.
    pub(super) fn workspaces(&self) -> Vec<Handle> {
