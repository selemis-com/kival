//! Wrapper for `std::fs` methods that provides additional path context on error.
...
/// # Errors
///
/// Returns an error if the symbolic link cannot be read.
...
/// # Errors
///
/// Returns an error if file removal fails for a reason other than the file not existing.
...
/// # Errors
///
/// Returns an error if the directory hierarchy cannot be created.
...
/// # Errors
///
/// Returns an error if the path cannot be renamed.
...
/// # Errors
///
/// Returns an error if metadata cannot be read for the path.
