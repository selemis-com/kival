...
mod tests {
    ...
    async fn workspace_admin_can_list_workspace_events(pool: sqlx::PgPool) -> Result<()> {
        ...
        let creator_membership = events
            .items
            .iter()
            .find(|event| {
                event.event_kind == "workspace.membership_created"
                    && event.workspace_id == Some(workspace.id)
                    && event.target_user_id == Some(r.admin.id)
            })
            .expect("workspace creation should audit the creator administrator membership");
        assert_eq!(creator_membership.payload["workspace_role"], serde_json::json!("admin"));
        assert!(
            creator_membership.payload["workspace_membership_id"]
                .as_str()
                .is_some_and(|id| Uuid::parse_str(id).is_ok()),
            "creator membership event should include its membership ID",
        );
        assert!(
            events
                .items
