...
mod tests {
    ...
    use kival_sdk::{
        ...
        Workspace, WorkspaceGraphResponse, WorkspaceGroup, WorkspaceGroupResponse,
    ...
    async fn execute(
        ...
        match operation {
            ...
            Operation::CreateWorkspace { output, name, .. } => {
                let response = actor
                    .browser
            ...
            Operation::ListWorkspaceMemberships { workspace, .. } => {
                ...
                let owner = reference.workspace_owner(*workspace).expect("modeled workspace owner");
                let owner_id = fixture.actors.get(owner).user_id;
                let owner_membership = response
                    .iter()
                    .find(|membership| membership.user_id == owner_id)
                    .expect("workspace owner has its intrinsic admin membership row");
                assert_eq!(
                    owner_membership.workspace_role,
                    kival_sdk::MembershipRole::Admin,
                    "workspace owner remains an administrator"
                );
                let actual = response
                    .iter()
                    .filter(|membership| membership.user_id != owner_id)
            ...
            Operation::RevokeWorkspaceMembership { workspace, membership, .. } => {
                ...
                assert_http_outcome(revoked_access, ExpectedOutcome::Forbidden, operation).await;
                assert_workspace_visibility_and_access(
                    fixture, resources, &api, namespace, reference, *workspace, operation,
                )
                .await;
