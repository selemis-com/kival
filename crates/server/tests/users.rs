...
mod tests {
    ...
    use eyre::Result;
    use kival_sdk::{
        ListResponse, UpdateUserRequest, User, UserResponse, UserStatus, WhoamiResponse,
    };
    ...
    #[sqlx::test(migrations = "../kernel/migrations")]
    async fn non_global_admin_cannot_update_self_display_name(pool: sqlx::PgPool) -> Result<()> {
        let r = TestKival::new(pool).await?;
        let user = r.create_user("user-update-self-denied").await?;
        ...
        let response = r
            .request_json_raw_as(
                ...
                &UpdateUserRequest { display_name: Some("Denied Self Update".to_owned()) },
            )
            .await?;
    ...
    async fn global_admin_can_enable_user_without_replacing_session_or_passkey(
        ...
        let restored: WhoamiResponse =
            r.get_json_as(&user, "/auth/whoami").await?.into_success()?;
