...
mod tests {
    ...
    async fn api_key_can_resolve_own_identity_without_an_identity_scope(
        ...
        assert_eq!(response.scopes, Some(vec![ApiKeyScope::ObjectRead]));
        Ok(())
    }
    ...
    #[sqlx::test(migrations = "../kernel/migrations")]
    async fn api_key_revocation_requires_fresh_authentication(pool: sqlx::PgPool) -> Result<()> {
        let r = TestKival::new(pool).await?;
        let workspace = r.create_workspace("api-key-revoke-fresh").await?;
        let created =
            create_api_key(&r, &r.admin, vec![ApiKeyScope::WorkspaceRead], vec![workspace.id])
                .await?;
        sqlx::query(
        ...
        .execute(&r.pool)
        .await?;
        let path = format!("/auth/api-keys/{}/revoke", created.api_key.id);

        let stale = r.request(Some(&r.admin), Method::POST, &path, None).await?;
        assert_eq!(stale.status(), StatusCode::FORBIDDEN);

        mark_actor_fresh(&r, &r.admin).await?;
        let fresh: ApiKeyResponse =
            r.empty_json_as(&r.admin, Method::POST, &path).await?.into_success()?;
        assert!(fresh.api_key.revoked_at.is_some());
