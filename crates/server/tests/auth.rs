...
mod tests {
    ...
    use kival_sdk::{
        API_PREFIX, MembershipRole, SessionListResponse, SessionOnlyResponse, UserResponse,
    };
    ...
    async fn session_can_resolve_own_identity(pool: sqlx::PgPool) -> Result<()> {
        ...
        let response: UserResponse = r.get_json_as(&actor, "/auth/whoami").await?.into_success()?;

        assert_eq!(response.user.id, actor.id);
        assert_eq!(response.is_global_admin, Some(false));
        assert_eq!(response.can_manage_groups, Some(false));
        Ok(())
    }
    ...
    async fn whoami_reports_group_management_capability(pool: sqlx::PgPool) -> Result<()> {
        ...
        let group_identity: UserResponse =
            r.get_json_as(&group_admin, "/auth/whoami").await?.into_success()?;
        assert_eq!(group_identity.is_global_admin, Some(false));
        assert_eq!(group_identity.can_manage_groups, Some(true));

        let global_identity: UserResponse =
            r.get_json_as(&r.admin, "/auth/whoami").await?.into_success()?;
        assert_eq!(global_identity.is_global_admin, Some(true));
        assert_eq!(global_identity.can_manage_groups, Some(true));
