...
use kival_sdk::{ListParams, ListResponse, UpdateUserRequest, User, UserListParams, UserResponse};
use serde_json::json;
use uuid::Uuid;
...
pub(crate) async fn handle_get_current_user(
    ...
    Ok(Json(UserResponse {
        user,
        is_global_admin: Some(global_admin),
        can_manage_groups: Some(can_manage_groups),
    }))
}
...
pub(crate) async fn handle_update_user(
    ...
    if actor.id != user_id && !is_global_admin(state.db(), actor.id).await? {
        return Err(ApiError::forbidden("global admin access required"));
    }
