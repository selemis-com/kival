...
impl ApiError {
    ...
    /// Creates a generic resource-state `409 Conflict` error.
    pub(crate) fn conflict(message: impl Into<String>) -> Self {
        Self::new(StatusCode::CONFLICT, "resource.conflict", message)
    }

    /// Creates an optimistic-version `409 Conflict` error.
    pub(crate) fn version_conflict(message: impl Into<String>) -> Self {
        Self::new(StatusCode::CONFLICT, "version.conflict", message)
    }

    /// Creates an invalid-pagination-cursor response.
    pub(crate) fn invalid_cursor(message: impl Into<String>) -> Self {
        Self::new(StatusCode::BAD_REQUEST, "invalid.cursor", message)
    }

    /// Creates an object-specific not-found response.
    pub(crate) fn object_not_found(message: impl Into<String>) -> Self {
        Self::new(StatusCode::NOT_FOUND, "object.not_found", message)
    }
    ...
    /// Converts a kernel error from an operation whose missing resource is the addressed object.
    pub(crate) fn from_object_kernel(error: KernelError) -> Self {
        match error {
            KernelError::ResourceNotFound => Self::object_not_found("object not found"),
    ...
    fn from(error: KernelError) -> Self {
        match error {
            ...
            KernelError::ObjectVersionConflict => {
                Self::version_conflict("object changed since the expected version")
            }
