...
pub(crate) fn decode_created_at(
    ...
    match cursor {
        ...
        _ => Err(ApiError::invalid_cursor("invalid pagination cursor")),
    }
}
...
pub(crate) fn decode_search(
    ...
    match cursor {
        Cursor::Search {
            ...
            if !rank.is_finite() {
                return Err(ApiError::invalid_cursor("invalid pagination cursor"));
            }
...
fn decode(cursor: Option<&str>) -> ApiResult<Option<Cursor>> {
    ...
    let bytes = URL_SAFE_NO_PAD
        .decode(cursor)
        .map_err(|_| ApiError::invalid_cursor("invalid pagination cursor"))?;
